...
import { createPost, updatePost } from "../../repository/PostRepository";
import { ErrorMessage } from "../../utils/ErrorMessage";
import { CommunityStackParamLst } from "../../types";

type IType = {
    params: CommunityStackParamLst["PostEdit"];
};
...
export default function PostEditScreen() {
    ...
    const handleUpdatePost = () => {
        setLoading(true);
        updatePost(post.pk, post.image ? post.image! === image ? "" : image : image, text).catch((error: any) => {
            if (
                error.response &&
                error.response.data &&
    ...
    return <ViewWithLoading loading={loading}>
        <View style={styles.container}>
            ...
            <ButtonComponent title="Save" onPress={async () => {
                handleUpdatePost();
            }} />
        </View>
    </ViewWithLoading>;
